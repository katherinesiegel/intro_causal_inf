...
```{r}
### open data
colo_dat <- read.csv("C:/Users/kjsie/Documents/ebio_course_data/colo_data_for_matching.csv")

### Make protection a factor variable
colo_dat <- colo_dat %>%
  mutate_at(vars(prot_cat_recl), 
            funs(factor))

### Generate weights
weights_colo_dat <- ipwpoint(exposure = prot_cat_recl,
                             family = "binomial", 
                             link = "logit",
                             denominator = ~ slope + elev_km + aspect_srai + lon + lat + dist_rds_km + popdens_1990 + lightning_5 + vs_max_fall_5 + vs_max_winter_5 + vs_max_spring_5 + vs_max_summer_5 + pr_total_fall_5 + pr_total_winter_5 + pr_total_spring_5 + pr_total_summer_5 + tmmx_avg_fall_5 + tmmx_avg_winter_5 + tmmx_avg_spring_5 + tmmx_avg_summer_5 + tmmn_avg_fall_5 + tmmn_avg_winter_5 + tmmn_avg_spring_5 + tmmn_avg_summer_5 + pdsi_avg_fall_5 + pdsi_avg_winter_5 + pdsi_avg_spring_5 + pdsi_avg_summer_5 + soil_avg_fall_5 + soil_avg_summer_5, 
                             data = colo_dat)

### Extract weights
colo_dat <- colo_dat %>%
  mutate(ipw = weights_colo_dat$ipw.weights) %>%
  dplyr::select(UID, ipw)

### Open full dataset
colo_full <- read.csv("C:/Users/kjsie/OneDrive/Documents/colo_dat_full.csv")

### Add weights to full dataset
colo_full <- merge(colo_full, colo_dat, by = "UID", all.x = TRUE)

### Make factor variables
colo_full <- colo_full %>%
  mutate_at(vars(burned, prot_cat_recl), 
            funs(factor))
...
### Estimate ATE
model_ipwpoint <- lm(burned ~ prot_cat_recl + #year +
                       slope + aspect_srai +
                       dist_rds_km + pdsi_avg_summer +
                       tmmx_avg_summer + vs_max_summer +
                       total_precip_summer + prev_yr_precip,
                     data = colo_full,
                     weights = ipw)
